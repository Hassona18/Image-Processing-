...
class ImageProcessorApp:
    ...
    def create_gui(self):
        ...
        tk.Button(operations_frame, text="Convert to Grayscale", command=self.convert_grayscale).pack(pady=5)
        tk.Button(operations_frame, text="Apply Threshold", command=self.apply_threshold).pack(pady=5)
        tk.Button(operations_frame, text="Simple Halftone", command=self.simple_halftone).pack(pady=5)
        tk.Button(operations_frame, text="Advanced Halftone", command=self.advanced_halftone).pack(pady=5)
        tk.Button(operations_frame, text="Histogram Equalization", command=self.histogram_equalization).pack(pady=5)

        tk.Button(operations_frame, text="Sobel Edge Detection", command=self.edge_detection_sobel).pack(pady=5)
        tk.Button(operations_frame, text="Prewitt Edge Detection", command=self.edge_detection_prewitt).pack(pady=5)
        tk.Button(operations_frame, text="Kirsch Edge Detection", command=self.edge_detection_kirsch).pack(pady=5)

        tk.Button(operations_frame, text="Apply High-pass Filter", command=self.high_pass_filter).pack(pady=5)
        tk.Button(operations_frame, text="Apply Low-pass Filter", command=self.low_pass_filter).pack(pady=5)
        tk.Button(operations_frame, text="Apply Median Filter", command=self.median_filter_op).pack(pady=5)

        tk.Button(operations_frame, text="Invert Image", command=self.invert_image).pack(pady=5)
        tk.Button(operations_frame, text="Add Image and Copy", command=self.add_images).pack(pady=5)
        tk.Button(operations_frame, text="Subtract Image and Copy", command=self.subtract_images).pack(pady=5)

        # Histogram Based Segmentation
        tk.Button(operations_frame, text="Manual Histogram Segmentation", command=self.manual_histogram_segmentation).pack(pady=5)
        tk.Button(operations_frame, text="Histogram Peak Segmentation", command=self.histogram_peak_segmentation).pack(pady=5)
        tk.Button(operations_frame, text="Histogram Valley Segmentation", command=self.histogram_valley_segmentation).pack(pady=5)
        tk.Button(operations_frame, text="Adaptive Histogram Segmentation", command=self.adaptive_histogram_segmentation).pack(pady=5)
    ...
    def edge_detection_kirsch(self):
        ...
        # Clip the values to ensure they fall within the valid range (0 to 255)
            max_magnitude = np.clip(max_magnitude, 0, 255)

        # Convert the result to an image and display
            self.processed_image = Image.fromarray(max_magnitude.astype(np.uint8))
            self.display_image(self.processed_image, "processed")
    ...
    def add_images(self):
        if self.uploaded_image:
            ...
            self.processed_image = Image.fromarray(np.clip(added, 0, 255).astype(np.uint8))
            self.display_image(self.processed_image, "processed")
    ...
    def subtract_images(self):
        if self.uploaded_image:
            ...
            self.processed_image = Image.fromarray(np.clip(subtracted, 0, 255).astype(np.uint8))
            self.display_image(self.processed_image, "processed")
